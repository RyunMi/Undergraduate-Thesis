...
$\mathcal{X}$ :特征空间； $\mathcal{Y}$ :标签空间；给定一组训练图像示例 $\mathcal{D}=\left\{\mathrm{x}\_i \in \mathcal{X} \subset \mathbb{R}^d, i=1,2, \cdots, n, n=l+u. \text{with} .l \ll u\right\}$ ，其中前 $l$ 个图像示例被标记为 $\{y_i\}^l_{i=1}\in \mathcal{Y}=\{1,2,\cdots,c\}$ ，其中 $c$ 是已知类的数量，并且剩余的 $u$ 个图像示例是未标记的。
