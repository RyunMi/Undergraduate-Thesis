...
其中 $S(x;\tau)^{(t)}$ 表示缩放标签预测，其元素在第 $t$ 次迭代时计算。 $\hat{S}(x;\tau)^{(t)}$ 和 $\hat{S}(x;\tau)^{(t-1)}$ 分别表示第 $t$ 次迭代和 $t-1$ 次迭代时的组合标签预测。系数 $\eta\in[0,1]$ 是一个动量参数，决定了聚合与训练历史的距离。$\hat{S}(x;\tau)^{(t)}$ 中元素的最大值被称为稳定softmax分数。
