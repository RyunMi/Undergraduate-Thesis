...
其中， $S$ 和 $T$ 分别表示源域和目标域中的任务。 $A=[a_S,a_T]\in R^{d\times 2}$ 是一个参数矩阵。 $U$ 是用于将原始高维数据映射到低维表示的 $d\times d$ 正交矩阵（映射函数）。 $A$ 的 $(r,p)$ 范数定义为 $\lVert A\rVert _{r,p}\triangleq(\sum_{i=1}^d||a^i||_r^p)^{\frac{1}{p}}$ 。
