...
一个源域 $D_S$ 一个目标域 $D_T$ 。更具体地，将源域数据表示为 $D_S=\{(x_{S_1},y_{S_1}),\dots,(x_{S_{n_S}},y_{S_{n_S}})\}$ ，其中 $x_{S_i}\in X_S$ 是数据实例， $y_{S_i}\in Y_S$ 是相应的类标签。将目标域数据表示为 $D_T=\{(x_{T_1},y_{T_1}),\dots,(x_{T_{n_T}},y_{T_{n_T}})\}$ ，其中 $x_{T_i}\in X}_T$ 是数据实例， $y_{T_i}\in Y_T$ 是相应的输出。在大多数情况下， $0\leq n_T\ll n_S$ 。
