...
此外，丰富的未标记数据具有提高SSL中模型泛化性的效果。在开集情况下，类集 $\overline{\mathcal{C}}^l$ 上的模型泛化性受到极大限制，因为只有稀缺的标记私有数据可用，这使得这类数据容易受到扰动。与 $\overline{\mathcal{C}}^l$ 相反， $C$ 中类的模型学习是充足的，因其既有标记数据也有未标记数据可利用。因此： $\mathbb{E}\_{\mathbf{x} \sim p_{\mathcal{C}}^l} w_s(\mathbf{x})<\mathbb{E}\_{\mathbf{x} \sim p_{\overline{\mathcal{C}}^l}^l} w_s(\mathbf{x})$ 。
