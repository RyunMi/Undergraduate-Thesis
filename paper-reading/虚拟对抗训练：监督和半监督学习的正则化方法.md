...
图2显示了 $\mathcal{R}_{vadv}$ 的转变以及在没有使用VAT训练的基准NN(表示为'wo/VAT')和使用VAT训练的NN(表示为'w/VAT')的学习曲线。随着训练的进行，使用VAT训练的NN的 $\mathcal{R}_{vadv}$ 超过了基准的 $\mathcal{R}_{vadv}$ ，也就是说，在 $LDS$ 方面，使用VAT训练的模型比基准模型更加平稳。
