...
对于标记数据批次 $\mathcal{X}$ 中的每个 $x_b$ ，本文生成一个变换版本 $\hat{x}\_b=Augment(x_b)$ (算法第3行)。对于未标记数据批次U中的每个 $u_b$ ，本文生成 $K$ 个增强 $\hat{u}\_{b,k}=Augment(u_b)$ ，其中 $k\in(1,\cdots,K)$ (算法第5行)。这些分离的增强被用于生成每个 $u_b$ 的“猜测标签” $q_b$ 。
